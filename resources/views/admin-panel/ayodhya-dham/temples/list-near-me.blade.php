        ...
        <div class="block-header">
            <div class="row">
                <div class="col-lg-8 col-md-6 col-sm-12">
                    <h2>Details Near the Temple </h2>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('adminDashboard')}}"><i class="zmdi zmdi-home"></i> Dashboard</a></li>
                        <li class="breadcrumb-item active">Details Near the Temple List</li>
                    </ul>
                    <button class="btn btn-primary btn-icon mobile_menu" type="button"><i class="zmdi zmdi-sort-amount-desc"></i></button>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-12">
                    <!-- <button class="btn btn-primary btn-icon float-right right_icon_toggle_btn" type="button"><i class="zmdi zmdi-arrow-right"></i></button> -->
                    <a href="{{route('adminAddNearMe')}}"><button class="btn btn-primary float-right">Add Details Near the Temple</button></a>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#users-table').on('click', '.near-me-status', function() {
            var button = $(this);
            var nearMeId = button.data('id');
            var currentStatus = button.data('status');
            var newStatus = currentStatus == '1' ? '0' : '1';
            var confirmationMessage = currentStatus == '1'
                ? "Are you sure you want to deactivate this near me?"
                : "Are you sure you want to activate this near me?";
            if (confirm(confirmationMessage)) {
                $.ajax({
                url: '{{ route('adminNearMeUpdateStatus') }}',
                type: 'POST',
                headers: {
